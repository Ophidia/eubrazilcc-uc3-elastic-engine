...
int oph_json_from_json_string(oph_json **json, char **jstring) {
  ...
  json_t *wrapped_response = NULL;

	//GET SOURCE DATA FROM JSON_T
	json_unpack(jansson,"{s?o}","response",&wrapped_response);
  if(wrapped_response){


	  //GET SOURCE DATA FROM JSON_T
	  json_unpack(wrapped_response,"{s?o}","source",&source);
	  if (source) {
	  ...
	  json_unpack(wrapped_response,"{s?o}","consumers",&consumers);
	  if (consumers) {
		  char *consumer=NULL;
		  size_t index;
	  ...
	  json_unpack(wrapped_response,"{s?o}","response",&response);
	  if (response) {
		  json_t *obj=NULL;
		  size_t index1;
